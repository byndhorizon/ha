...
public class ZigbeeMultiInstanceCommandClass extends ZigbeeCommandClass {

	private static final Logger logger = LoggerFactory.getLogger(ZigbeeMultiInstanceCommandClass.class);
	...
	/**
	 * Creates a new instance of the ZigbeeMultiInstanceCommandClass class.
	 * @param node the node this command class belongs to
	 * @param controller the controller to use
	 * @param endpoint the endpoint this Command class belongs to
	 */
	public ZigbeeMultiInstanceCommandClass(ZigbeeNode node,
	 ...
	 */
	public ZigbeeEndpoint getEndpoint(int endpointId){
	...
	private void handleMultiInstanceReportResponse(SerialMessage serialMessage,
		...
		} else 
		{
			...
			ZigbeeCommandClass zigbeeCommandClass = this.getNode().getCommandClass(commandClass);
			...
			zigbeeCommandClass.setInstances(instances);
		...
		int waiting = 0;
		for (ZigbeeCommandClass zigbeeCommandClass : this.getNode().getCommandClasses()) {
			// We never sent the NO_OP request, so ignore it here.
			if (zigbeeCommandClass.getCommandClass() == CommandClass.NO_OPERATION)
				continue;
			
			if (zigbeeCommandClass.getInstances() == 0) {
				// Still waiting for an instance report of another command class. 
				waiting++;
				logger.debug("NODE {}: Waiting for command class {}.", this.getNode().getNodeId(), zigbeeCommandClass.getCommandClass().getLabel());
			}
		}
		if(waiting != 0) {
	...
	private void handleMultiInstanceEncapResponse(
		...
		zigbeeCommandClass.handleApplicationCommandRequest(serialMessage, offset + 2, instance);
	}
	...
	private void handleMultiChannelEndpointReportResponse(
		...
		for (int i=1; i <= endpoints; i++) {
			ZigbeeEndpoint endpoint = new ZigbeeEndpoint(i);
	...
	private void handleMultiChannelCapabilityReportResponse(SerialMessage serialMessage, int offset) {
		...
		for (int endpointId = startId; endpointId <= endId; endpointId++) {
			ZigbeeEndpoint endpoint = this.endpoints.get(endpointId);
			...
			ZigbeeDeviceClass deviceClass = endpoint.getDeviceClass();
			...
			if (supportsBasicCommandClass) {
				ZigbeeCommandClass commandClass = new ZigbeeBasicCommandClass(this.getNode(), this.getController(), endpoint);
			...
			for (int i = 0; i < serialMessage.getMessagePayload().length - offset - 3; i++) {
				...
				ZigbeeCommandClass commandClass = ZigbeeCommandClass.getInstance(data, this.getNode(), this.getController(), endpoint);
				...
				ZigbeeCommandClass parentClass = this.getNode().getCommandClass(commandClass.getCommandClass());
		...
		if (this.endpointsAreTheSameDeviceClass)
			...
			for (ZigbeeEndpoint ep : this.endpoints.values())
			{
	...
	private void handleMultiChannelEncapResponse(
		...
		ZigbeeCommandClass zigbeeCommandClass;
		...
		zigbeeCommandClass = endpoint.getCommandClass(commandClass);
		...
		if (zigbeeCommandClass == null) {
			...
			zigbeeCommandClass = this.getNode().getCommandClass(commandClass);
		}
		...
		zigbeeCommandClass.handleApplicationCommandRequest(serialMessage, offset + 3, endpointId);
	}
	 ...
	 */
	public SocketMessage getMultiInstanceGetMessage(CommandClass commandClass) {
	 ...
	 */
	public SocketMessage getMultiInstanceEncapMessage(SocketMessage serialMessage, int instance) {
	 ...
	 * @return the serial message.
	 */
	public SocketMessage getMultiChannelEndpointGetMessage() {
	 ...
	 * @return the serial message.
	 */
	public SocketMessage getMultiChannelCapabilityGetMessage(ZigbeeEndpoint endpoint) {
	 ...
	 * @return the encapsulated serial message.
	 */
	public SocketMessage getMultiChannelEncapMessage(SocketMessage serialMessage, ZigbeeEndpoint endpoint) {
	...
	public void initEndpoints() {
		...
		switch (this.getVersion()) {
			case 1:
				...
				for (ZigbeeCommandClass commandClass : this.getNode().getCommandClasses()) {
