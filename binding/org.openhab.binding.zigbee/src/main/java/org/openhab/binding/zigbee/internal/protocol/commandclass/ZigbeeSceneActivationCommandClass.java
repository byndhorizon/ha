...
public class ZigbeeSceneActivationCommandClass extends ZigbeeCommandClass {

	private static final Logger logger = LoggerFactory.getLogger(ZigbeeBasicCommandClass.class);
	
	private static final int SCENEACTIVATION_SET = 0x01;
	
	/**
	 * Creates a new instance of the ZigbeeSceneActivationCommandClass class.
	 * @param node the node this command class belongs to
	 * @param controller the controller to use
	 * @param endpoint the endpoint this Command class belongs to
	 */
	public ZigbeeSceneActivationCommandClass(ZigbeeNode node,
	...
	protected void processSceneActivationSet(SerialMessage serialMessage, int offset, int endpoint) {
		...
		ZigbeeCommandClassValueEvent zEvent = new ZigbeeCommandClassValueEvent(this.getNode().getNodeId(), endpoint, this.getCommandClass(), sceneId);
