...
/**
 * Zigbee Command Class. Zigbee device functions are controlled 
 ...
 * @since 1.3.0
 */
public abstract class ZigbeeCommandClass {

	private static final Logger logger = LoggerFactory.getLogger(ZigbeeCommandClass.class);
...
//	private static final SCALE_SHIFT = 0x03; // unused
	 ...
	 */
	protected ZigbeeCommandClass(ZigbeeNode node, ZigbeeController controller, ZigbeeEndpoint endpoint)	{
		this.node = node;
	 ...
	 */
	public void setController(ZigbeeController controller) {
		this.controller = controller;
	}
	 ...
	 */
	public void setEndpoint(ZigbeeEndpoint endpoint) {
		this.endpoint = endpoint;
	}
	 ...
	 * @param node the node this instance commands.
	 * @param controller the controller to send messages to.
	 * @return the ZigbeeCommandClass instance that was instantiated, null otherwise 
	 */
	public static ZigbeeCommandClass getInstance(int i, ZigbeeNode node, ZigbeeController controller) {
		return ZigbeeCommandClass.getInstance(i, node, controller, null);
	}
	 ...
	 * @return the ZigbeeCommandClass instance that was instantiated, null otherwise 
	 */
	public static ZigbeeCommandClass getInstance(int i, ZigbeeNode node, ZigbeeController controller, ZigbeeEndpoint endpoint) {
		...
		try {
			...
			Class<? extends ZigbeeCommandClass> commandClassClass = commandClass.getCommandClassClass();
			...
			Constructor<? extends ZigbeeCommandClass> constructor = commandClassClass.getConstructor(ZigbeeNode.class, ZigbeeController.class, ZigbeeEndpoint.class);
	 ...
	 * Taken from: http://wiki.micasaverde.com/index.php/Zigbee_Command_Classes
	 * @author Jan-Willem Spuij
	 * @since 1.3.0
	 */
	@XStreamAlias("commandClass")
	public enum CommandClass {
		NO_OPERATION(0x00,"NO_OPERATION", ZigbeeNoOperationCommandClass.class),
		BASIC(0x20,"BASIC",ZigbeeBasicCommandClass.class),
		...
		SWITCH_BINARY(0x25,"SWITCH_BINARY",ZigbeeBinarySwitchCommandClass.class),
		//SWITCH_MULTILEVEL(0x26,"SWITCH_MULTILEVEL",ZigbeeMultiLevelSwitchCommandClass.class),
		...
		SCENE_ACTIVATION(0x2B,"SCENE_ACTIVATION",ZigbeeSceneActivationCommandClass.class),
		...
		//SENSOR_BINARY(0x30,"SENSOR_BINARY",ZigbeeBinarySensorCommandClass.class),
		//SENSOR_MULTILEVEL(0x31,"SENSOR_MULTILEVEL",ZigbeeMultiLevelSensorCommandClass.class),
		//METER(0x32,"METER",ZigbeeMeterCommandClass.class),
		...
		//THERMOSTAT_SETPOINT(0x43,"THERMOSTAT_SETPOINT",ZigbeeThermostatSetpointCommandClass.class),
		...
		MULTI_INSTANCE(0x60,"MULTI_INSTANCE",ZigbeeMultiInstanceCommandClass.class),
		DOOR_LOCK(0x62,"DOOR_LOCK",null),
		USER_CODE(0x63,"USER_CODE",null),
		//CONFIGURATION(0x70,"CONFIGURATION",ZigbeeConfigurationCommandClass.class),
		ALARM(0x71,"ALARM",null),
		//MANUFACTURER_SPECIFIC(0x72,"MANUFACTURER_SPECIFIC",ZigbeeManufacturerSpecificCommandClass.class),
		...
		//BATTERY(0x80,"BATTERY",ZigbeeBatteryCommandClass.class),
		CLOCK(0x81,"CLOCK",null),
		HAIL(0x82,"HAIL",ZigbeeHailCommandClass.class),
		//WAKE_UP(0x84,"WAKE_UP", ZigbeeWakeUpCommandClass.class),
		//ASSOCIATION(0x85,"ASSOCIATION",ZigbeeAssociationCommandClass.class),
		//VERSION(0x86,"VERSION",ZigbeeVersionCommandClass.class),
		...
		//SENSOR_ALARM(0x9C,"SENSOR_ALARM",ZigbeeAlarmSensorCommandClass.class),
		...
		private Class<? extends ZigbeeCommandClass> commandClassClass;


		private CommandClass(int key, String label, Class<? extends ZigbeeCommandClass> commandClassClass) {
			this.key = key;
			this.label = label;
		...
		public static CommandClass getCommandClass(String label) {
			...
			System.out.println("getCommandClass!!!!!!!!!!");
			System.out.println(label.toLowerCase());
			System.out.println(labelToCommandClassMapping.get(label.toLowerCase()));
		 ...
		 */
		public Class<? extends ZigbeeCommandClass> getCommandClassClass() {
