...
import org.openhab.binding.zigbee.internal.converter.state.BooleanOnOffTypeConverter;
import org.openhab.binding.zigbee.internal.converter.state.BooleanOpenClosedTypeConverter;
import org.openhab.binding.zigbee.internal.converter.state.DateDateTimeTypeConverter;
import org.openhab.binding.zigbee.internal.converter.state.IntegerDecimalTypeConverter;
import org.openhab.binding.zigbee.internal.converter.state.IntegerPercentTypeConverter;
import org.openhab.binding.zigbee.internal.converter.state.StringStringTypeConverter;
...
/**
 * ZigbeeInfoConverter class. Converters between binding items
 * and the Zigbee API to gather statistic information.
 * @author Jan-Willem Spuij
 * @since 1.4.0
 */
public class ZigbeeInfoConverter extends ZigbeeConverterBase {

	private static final Logger logger = LoggerFactory.getLogger(ZigbeeInfoConverter.class);
	 ...
	 */
	public ZigbeeInfoConverter(ZigbeeController controller, EventPublisher eventPublisher) {
	 ...
	 */
	public void executeRefresh(Item item, ZigbeeNode node, int endpointId, Map<String,String> arguments) {
		if(item == null)
			return;
		...
		ZigbeeInformationItem informationItem = ZigbeeInformationItem.getZigbeeBindingAction(arguments.get("item"));
        ...
        if (value != null) {        
            ZigbeeStateConverter<?,?> converter = this.getStateConverter(item, value);
    ...
    private Object getInformationItemValue(ZigbeeNode node, ZigbeeInformationItem informationItem) {
	...
	/**
	 * Zigbee information item enumeration. It Defines the information that can be 
	 ...
	 * @since 1.3.0
	 */
	public enum ZigbeeInformationItem {
		...
		/**
		 * Reports whether the node is routing Zigbee messages.
		 */
		...
		private static Map<String, ZigbeeInformationItem> labelToZigbeeInfoItemMapping;

		private ZigbeeInformationItem(String label) {
			this.label = label;
		}
		
		private static void initMapping() {
			labelToZigbeeInfoItemMapping = new HashMap<String, ZigbeeInformationItem>();
			for (ZigbeeInformationItem s : values()) {
				labelToZigbeeInfoItemMapping.put(s.label.toLowerCase(), s);
			}
		}

		/**
		 * Returns the label of the ZigbeeInformationItem enumeration
		 * @return the label
		 */
		 ...
		 */
		public static ZigbeeInformationItem getZigbeeBindingAction(String label) {
			if (labelToZigbeeInfoItemMapping == null) {
				initMapping();
			}
			return labelToZigbeeInfoItemMapping.get(label.toLowerCase());
		}
