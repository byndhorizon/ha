...
public class ZigBeeNetworkManagerSerialImpl implements ZigBeeNetworkManager {
    ...
    public void startup() {
        ...
        if (state == DriverStatus.CLOSED) {
        	...
        	System.out.println("Driver already opened, current status");
    ...
    private boolean initializeHardware() {
    	System.out.println("initializeHardware 11111111111 "+port);
        zigbeeSerialInterface = new ZigBeeSerialInterface(port);
        System.out.println("initializeHardware 222222222222222222");
        if (!zigbeeSerialInterface.open()) {
        	System.out.println("initializeHardware 33333333333333333");
            logger.error("Failed to initialize the dongle on port {} at rate {}", port, rate);
            return false;
        }
        System.out.println("initializeHardware 44444444444444");
        return true;
    }
